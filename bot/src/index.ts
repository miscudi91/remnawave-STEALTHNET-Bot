...
async function enforceSubscription(
  ...
  await ctx.reply(`?? ${msg}`, { reply_markup: subscribeKeyboard(channelId, lang) });
  return true;
}
...
const DEFAULT_EMOJI_UNICODE: Record<string, string> = {
  PACKAGE: "??", TARIFFS: "??", CARD: "??", LINK: "??", PUZZLE: "рџвЂ?¤", PROFILE: "рџвЂ?¤",
...
bot.command("start", async (ctx) => {
  ...
  try {
    ...
    if (isBotAdmin) {
      markup.inline_keyboard.push([{ text: "?? Панель админа", callback_data: "admin:menu" }]);
    }
...
bot.on("callback_query:data", async (ctx) => {
  ...
  if (data.startsWith("admin:")) {
    ...
    if (data === "admin:menu") {
      ...
      await editMessageContent(ctx, "?? Панель админа\n\nВыберите раздел:", markup);
      return;
    }
    ...
    if (data === "admin:search") {
      ...
      await editMessageContent(
        ...
        { inline_keyboard: [[{ text: "?? Отмена", callback_data: "admin:menu" }]] }
      );
      return;
    }
    if (data === "admin:stats") {
      ...
      const back: InlineMarkup = { inline_keyboard: [[{ text: "?? В админку", callback_data: "admin:menu" }]] };
    ...
    if (data.startsWith("admin:clients:")) {
      ...
      if (suffix === "clear") {
        ...
        nav.push({ text: "?? В админку", callback_data: "admin:menu" });
      ...
      const totalPages = Math.max(1, Math.ceil(total / limit));
      let msg = search ? `рџвЂ?Ґ Поиск «${search}» (${total})\n\n` : `рџвЂ?Ґ Клиенты (${total})\n\n`;
      items.forEach((c, i) => {
        const label = c.email || c.telegramUsername || c.telegramId || c.id.slice(0, 8);
    ...
    if (data.startsWith("admin:client:")) {
      ...
      kb.push([{ text: "?? К списку", callback_data: "admin:clients:1" }]);
    ...
    if (data.startsWith("admin:remna:revoke:")) {
      ...
      try {
        ...
        await editMessageContent(ctx, `? Подписка Remna отозвана для клиента.`, {
          inline_keyboard: [[{ text: "?? К клиенту", callback_data: `admin:client:${clientId}` }]],
        });
      } catch (e: unknown) {
        await editMessageContent(ctx, `? ${e instanceof Error ? e.message : "Ошибка"}`, {
          inline_keyboard: [[{ text: "?? Назад", callback_data: `admin:client:${clientId}` }]],
        });
      }
      return;
    }
    ...
    if (data.startsWith("admin:squad:add:")) {
      ...
      if (indexStr !== undefined) {
        ...
        try {
          ...
          await editMessageContent(ctx, `? ${e instanceof Error ? e.message : "Ошибка"}`, {
            inline_keyboard: [[{ text: "?? Назад", callback_data: `admin:squad:add:${clientId}` }]],
          });
        }
        return;
      }
      try {
        ...
        rows.push([{ text: "?? К клиенту", callback_data: `admin:client:${clientId}` }]);
    ...
    if (data.startsWith("admin:squad:remove:")) {
      ...
      if (indexStr !== undefined) {
        ...
        try {
          ...
          await editMessageContent(ctx, `? ${e instanceof Error ? e.message : "Ошибка"}`, {
            inline_keyboard: [[{ text: "?? Назад", callback_data: `admin:squad:remove:${clientId}` }]],
          });
        }
        return;
      }
    ...
    if (data.startsWith("admin:pay:")) {
      ...
      try {
        ...
        await editMessageContent(ctx, "? Платёж отмечен как оплаченный.", {
          inline_keyboard: [[{ text: "?? К платежам", callback_data: "admin:payments:pending:1" }]],
        });
      } catch (e: unknown) {
        await editMessageContent(ctx, `? ${e instanceof Error ? e.message : "Ошибка"}`, {
          inline_keyboard: [[{ text: "?? Назад", callback_data: "admin:payments:pending:1" }]],
        });
      }
      return;
    }
    ...
    if (data.startsWith("admin:bc:")) {
      ...
      if (raw == null) {
        await editMessageContent(ctx, "Текст рассылки не найден. Начните заново.", {
          inline_keyboard: [[{ text: "?? В админку", callback_data: "admin:menu" }]],
        });
        return;
      }
    ...
    if (data.startsWith("admin:block:")) {
      ...
      const kb: InlineMarkup["inline_keyboard"] = [
        ...
        [{ text: "?? К списку", callback_data: "admin:clients:1" }],
      ];
      await editMessageContent(ctx, text, { inline_keyboard: kb });
      return;
    }
  ...
  try {
    ...
    if (config?.forceSubscribeEnabled && config.forceSubscribeChannelId?.trim()) {
      ...
      if (result.state !== "subscribed") {
        ...
        const details = result.state === "cannot_verify"
          ? `\n\n${_t("subscribe.cannot_verify", lang)}`
          : "";
        await editMessageContent(ctx, `?? ${msg}${details}`, subscribeKeyboard(channelId, lang));
        return;
      }
    }
    ...
    if (data === "menu:main") {
      ...
      if (config?.botAdminTelegramIds?.includes(String(userId))) {
        backMarkup.inline_keyboard.push([{ text: "?? Панель админа", callback_data: "admin:menu" }]);
      }
    ...
    if (data === "menu:devices") {
      ...
      try {
        ...
        rows.push([{ text: config?.botBackLabel ?? "?? В меню", callback_data: "menu:main" }]);
        await editMessageContent(ctx, lines.join("\n"), { inline_keyboard: rows });
      } catch (e: unknown) {
        const msg = e instanceof Error ? e.message : "Ошибка";
        await editMessageContent(ctx, `?? Устройства\n\n? ${msg}`, {
          inline_keyboard: [[{ text: config?.botBackLabel ?? "?? В меню", callback_data: "menu:main" }]],
        });
      }
      return;
    }
    ...
    if (data.startsWith("devices:delete:")) {
      ...
      try {
        ...
        await editMessageContent(ctx, `? ${e instanceof Error ? e.message : "Ошибка"}`, {
          inline_keyboard: [[{ text: config?.botBackLabel ?? "?? В меню", callback_data: "menu:devices" }]],
        });
      }
      return;
    }
    ...
    if (data.startsWith("gift_pay_balance:")) {
      ...
      try {
        ...
        await editMessageContent(
          ctx,
          `? Дополнительная подписка создана!\n\nПодписка #${result.subscriptionIndex}\n\nВы можете активировать еСвЂ? на своСвЂ?м аккаунте или подарить другу.`,
    ...
    if (data === "gift:subscriptions") {
      try {
        ...
        rows.push([{ text: config?.botBackLabel ?? "?? Назад", callback_data: "menu:gift" }]);
        await editMessageContent(
          ctx,
      ...
      try {
        ...
        if (config?.useRemnaSubscriptionPage) {
          ...
          await editMessageContent(
            ...
            {
              inline_keyboard: [
                [{ text: "?? Подключиться к VPN", url: remnaUrl }],
...
bot.on("message:text", async (ctx) => {
  ...
  if (awaitingAdminSearch.has(userId)) {
    ...
    try {
      ...
      const totalPages = Math.max(1, Math.ceil(total / limit));
      const msg =
        (searchQuery ? `рџвЂ?Ґ Поиск «${searchQuery}» (${total})\n\n` : `рџвЂ?Ґ Клиенты (${total})\n\n`) +
        items
          .map(
            (c, i) =>
